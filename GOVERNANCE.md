# Governance

CodeLane is currently maintained as a small project with a lightweight maintainer-led governance model.

## Maintainer responsibilities

Maintainers are responsible for:

- setting project direction
- reviewing pull requests
- reviewing and organizing issues
- maintaining architecture boundaries
- responding to security reports
- enforcing the code of conduct
- keeping CI and repository tooling healthy
- deciding what belongs in the starter kit

## Decision making

Most decisions can happen in issues and pull requests.

For larger changes, prefer opening a discussion or issue first. Examples:

- changing package boundaries
- replacing core tooling
- adding a new deployable app
- changing authentication architecture
- changing licensing
- introducing new required infrastructure
- large UI/design system shifts

Maintainers make final decisions when consensus is unclear.

## Contribution review

Pull requests are reviewed for:

- correctness
- maintainability
- test coverage appropriate to the risk
- documentation updates
- package boundary alignment
- security impact
- consistency with project goals

Maintainers may ask for changes or decline a contribution if it is outside the project's scope.

## Project scope

CodeLane is a monorepo starter kit for cross-platform TypeScript applications with a shared backend.

It is not intended to become:

- a marketplace
- a generic package ecosystem
- a large business application by default
- a collection of unrelated examples
- a replacement for framework documentation

## Becoming a maintainer

There is no formal maintainer nomination process yet. If the project grows, maintainership can be expanded based on sustained, constructive contributions and trust.
